...
@section('contenidoprincipal')
    ...
    @if ($tipouser == 1)
        <div class="container" style="padding-left: 150px;padding-top:10px;">
            ...
            <hr>
            <form action="{{ route('updateusuario', $usuario->id) }}" method="POST" id="formulariousuario"
                name="formulariousuario" class="form">
                @method('PUT')
                @csrf
                <div class="form-group">
                    <label for="name" class="form-label">Nickname</label>
                    <input type="text" name="name" id="name" class="form-control" placeholder="user@1994" tabindex="1"
                        autocomplete="off" autofocus value="{{ $usuario->name }}">
                    <span style="color: red;">@error('name'){{ $message }}@enderror</span>
                    </div>
                    <div class="form-group">
                        <label for="email" class="form-label">Email</label>
                        <input type="email" name="email" id="email" class="form-control" placeholder="Example@gmail.com"
                            tabindex="2" autocomplete="off" value="{{ $usuario->email }}">
                        ...
                        <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <button type="submit" class="btn btn-outline-warning btn-lg" style="margin-top: 10px;">Editar</button>
                            </form>
                        </div>
                    @else
